...
export const MobileOperatorSalesBLE = () => {
  console.log('MobileOperatorSalesBLE component rendering');
  ...
  const handleCreateSale = async () => {
    ...
    // Check if liters exceed available stock
    const totalStock = getTotalStockByFuelType(formData.fuel_type)
    if (formData.liters > totalStock) {
      toast.error(`Requested liters (${formatNumber(formData.liters)}L) exceed available stock (${formatNumber(totalStock)}L)`)
      return
    }
        ...
        <CardContent className="pb-6">
          <div className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                <input
                  ...
                  className="flex h-14 w-full rounded-lg border-2 border-gray-300 bg-background px-4 py-3 text-base ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:border-blue-500 disabled:cursor-not-allowed disabled:opacity-50"
                />
                <p className="text-sm text-gray-500">
                ...
                {formData.fuel_type && formData.fuel_type !== '__no_fuel_types__' && formData.liters > 0 && (
                  <div className={`p-3 rounded-lg border ${formData.liters > getTotalStockByFuelType(formData.fuel_type) ? 'bg-red-50 border-red-200 text-red-700' : 'bg-green-50 border-green-200 text-green-700'}`}>
                    <div className="flex items-center gap-2">
                      {formData.liters > getTotalStockByFuelType(formData.fuel_type) ? (
                        <>
                          <AlertTriangle className="h-5 w-5 flex-shrink-0" />
                          <span className="font-medium">⚠️ Requested liters exceed available stock!</span>
                        </>
                      ) : (
                        <>
            ...
            <Button 
              ...
              className="h-14 text-base py-4 rounded-lg shadow-md hover:shadow-lg transition-shadow"
            >
              {submitting ? (
                <div className="flex items-center">
