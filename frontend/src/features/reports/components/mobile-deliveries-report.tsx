...
import { TruckIcon, DownloadIcon } from 'lucide-react'
...
interface MobileDeliveriesReportProps {
  reportData: DeliveriesReportData
...
export const MobileDeliveriesReport: React.FC<MobileDeliveriesReportProps> = ({
  reportData,
  spbuOptions,
  selectedSpbu,
  dateRange,
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="flex items-center">
          ...
          <span className="text-base">Loading deliveries report...</span>
        </div>
      </div>
    )
  }
      ...
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <div className="text-gray-600 mb-4 text-sm">
            Failed to load deliveries report data. Please try again later.
          </div>
    ...
    <div className="flex flex-col gap-6 pb-4">
      ...
      <div className="px-4 pt-2">
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Track fuel deliveries and supplier performance
        </p>
      </div>
      ...
      {reportData.data && reportData.data.length > 0 && (
        <div className="px-4">
          <Card className="shadow-sm">
            <CardHeader className="pb-4">
              <CardTitle>Delivery Transactions</CardTitle>
              <CardDescription>
                Detailed list of delivery transactions
              </CardDescription>
            </CardHeader>
                    ...
                    <CardContent className="p-4">
                      ...
                      <div className="grid grid-cols-1 gap-2 text-sm">
                        ...
                        <div className="flex flex-wrap items-center gap-1">
                          ...
                          <span>{(item.planned_liters || 0).toLocaleString('id-ID', { minimumFractionDigits: 2 })} L</span>
                        </div>
                        <div className="flex flex-wrap items-center gap-1">
                          <span className="text-muted-foreground">Actual:</span>
                          <span>{item.actual_liters !== null ? item.actual_liters.toLocaleString('id-ID', { minimumFractionDigits: 2 }) : '-'}</span>
                        </div>
                        <div className="flex flex-wrap items-center gap-1">
                          <span className="text-muted-foreground">Price/Liter:</span>
                          <span>{formatCurrency(item.pricePerLiter)}</span>
                        </div>
                        <div className="flex flex-wrap items-center gap-1">
                          <span className="text-muted-foreground">Harga Beli:</span>
                          <span>{item.harga_beli ? formatCurrency(item.harga_beli) : '-'}</span>
                        </div>
                        <div className="flex flex-wrap items-center gap-1">
                          <span className="text-muted-foreground">Total Cost:</span>
                          <span>{formatCurrency(item.totalHargaBeli)}</span>
                        </div>
                        <div className="flex flex-wrap items-center gap-1">
                          <span className="text-muted-foreground">Confirmed:</span>
                          <span className="truncate">{item.confirmedBy || '-'}</span>
                        </div>
                      </div>
                    </CardContent>
                  </Card>
